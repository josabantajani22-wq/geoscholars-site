    ...
    async studentDetail(uid) {
      ...
      box.innerHTML = `<div class="stu-detail">
        ...
        <div class="profile-grid" style="margin-top:1rem">
          ...
          <div class="card"><h3>GSA courses</h3>${(u.courses || []).length ? `<ul class="clean">${u.courses.map(id => `<li>${esc(((window.GSA_SITE || {}).courses || []).find(c => c.id === id)?.name || id)}</li>`).join("")}</ul>` : "<p class='small muted'>None added by the student.</p>"}</div>
        </div>
